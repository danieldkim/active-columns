...
var suite = wrap({
  ...
  suite: {
    "Mutations for Users1": function (test) {
      ...
      [ 
        ...
        test.ok(_.any(mutations, function(mut) {
    ...
    "Mutations for Users2": function (test) {
    ...
    "Mutations for StateUsers1": function (test) {
      ...
      expected_mutations.forEach(function(exp_mut) {
        test.ok(_.any(mutations, function(mut) {
          ...
          exp_mut.columns.forEach(function(exp_col) {
            test.ok(_.any(mut.columns, function(col) {
      ...
      test.equal(1, mutations.length);
      var exp_mut = expected_mutations[0], mut = mutations[0];
      test.equal(exp_mut.super_column, mut.super_column, 
    ...
    "Mutations for StateUsers2": function (test) {
      ...
      test.ok(exp_mut.name == mut.name && exp_mut.value == mut.value && 
    ...
    "Mutations for StateLastLoginUsers": function (test) {
      ...
      exp_mut.columns.forEach(function(exp_col) {
        test.ok(_.any(mut.columns, function(col) {
          ...
          for (var k in exp_val) {
            var v = exp_val[k];
            test.equal(v, col_val[k], "Expected " + k + " to be " + v);
          }
          return true;
        }), 
        "Mutations for super column " + mut.super_column + 
...
module.exports = { "Mutations tests": suite }